...
func getOneToOneNat(client *opnsense.Client, uuid string) (*oneToOneNat, error) {
	...
	if reflect.DeepEqual(response, getOneToOneNatResponse{}) {
		return nil, fmt.Errorf("get one-to-one NAT error: one-to-one NAT rule with uuid `%s` does not exist", uuid)
	}
